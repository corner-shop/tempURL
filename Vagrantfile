...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    # on the first boot, /var/lib/apt may be empty
    x=$(find  /var/lib/apt) && if [ "$x" == "" ]; then sudo apt-get update; fi
    # only update once every 24h
    x=$(find  /var/lib/apt -ctime 1) && if [ "$x" != "" ]; then sudo apt-get update; fi
